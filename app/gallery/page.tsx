    ...
    <div className="pt-20 pb-16 bg-black min-h-screen">
      ...
      {selectedItem && (
        <div
          ...
          onClick={handleOutsideClick}
        >
          {/* Changed max-w-5xl to max-w-7xl to let widescreen layouts grow fully */}
          <div className="max-w-7xl w-full relative">
            ...
            <div className="bg-zinc-950 rounded-2xl overflow-hidden shadow-2xl">
              {selectedItem.type === "image" ? (
                ...
                /* Changed to aspect-video w-full to stretch cleanly into cinematic screen width */
                <video
                  controls
                  autoPlay
                  preload="metadata"
                  poster={selectedItem.thumbnail}
                  className="w-full aspect-video max-h-[80vh] object-cover rounded-t-2xl"
                >
                  {(selectedItem.sources ?? [{ src: selectedItem.src, type: "video/mp4" }]).map(
                    (source) => (
                      <source key={source.src} src={source.src} type={source.type} />
